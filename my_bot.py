...
async def generate_first_question() -> str:
    ...
    Формат: только вежливый вопрос с кратким приветствием, без пояснений.
    """
    try:
...
async def generate_technical_question(chat_history: list[dict]) -> str:
    ...
    Формат: только вопрос, без пояснений.
    """
    try:
        response = await llm.ainvoke([HumanMessage(content=prompt)])
        return response.content.strip()
...
async def determine_final_position(chat_history: list[dict]) -> Position:
    ...
    Подходит ли кандидат на какую-то из этих позиций (кандидат не обязательно должен подходить, тогда он нам не подходит):
...
async def should_end_early(chat_history: list[dict]) -> tuple[bool, Position]:
    ...
    Если у кандидата нет всех подходящих навыков ни для одной вакансии (нужно проверить все необходимые технические навыки), то ответь: Нет
